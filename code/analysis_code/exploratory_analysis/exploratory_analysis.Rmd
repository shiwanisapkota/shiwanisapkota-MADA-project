...
data_clean_maint$hispanic_status <- recode_factor(data_clean_maint$hispanic_status, '1' = 'NH', '2' = 'H')
...
# Lets look at the distribution of hipanic and mammogram status
CrossTable(data_clean_maint$year, data_clean_maint$mammogram_status)

# Lets look at the hispanic status and mammogram status
CrossTable(data_clean_maint$hispanic_status, data_clean_maint$mammogram_status)

# Lets look at the income and mammogram status
...
# Lets look at the marital status and mammogram status
...
# Lets look at the insurance status and mammogram status
CrossTable(data_clean_maint$insurance_status, data_clean_maint$mammogram_status)

# Lets look at the health status and mammogram status
CrossTable(data_clean_maint$health_status, data_clean_maint$mammogram_status)

# Lets look at the usual medical care status and mammogram status
...
# Lets look at the alcohol status and mammogram status
CrossTable(data_clean_maint$alcohol_status, data_clean_maint$mammogram_status)

# Lets look at the diabetes status and mammogram status
...
# Creating a demographic characteristics table on the basis of mammogram screening status
...
# Saving this figure
ggsave(filename = here("results", "exploratory_analysis_result", "figure1_mammogram_ses.png"), plot = figure1)
```
